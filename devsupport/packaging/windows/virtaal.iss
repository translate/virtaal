...
; Scoped to po/LINGUAS ∩ Inno's own isl files (#3693), not the full
; po/ directory or Inno's whole language list. No [Code] needed - Inno's
; defaults (LanguageDetectionMethod=uilanguage, ShowLanguageDialog=yes)
; already auto-select by Windows UI language while letting the user
; override.
;
; All non-English files are vendored into .\languages\, none referenced
; via compiler:Languages\ - CI's installed Inno Setup (via choco) turned
; out not to bundle every language jrsoftware/issrc's own source ships
; (ChineseTraditional.isl specifically was missing), so relying on
; "should be bundled" isn't safe. Vendoring from the same upstream
; source for every language, official or not, sidesteps whatever a
; given Inno Setup install happens to include.
[Languages]
...
; Unofficial (no Inno-bundled equivalent at all, official or not).
; Translator-credit headers kept intact (required by Inno's own
; redistribution license). Actively maintained (targets Inno 6.4.0+/6.5.0+):
...
; Generated by generate_installer_translations.py (build_installer.ps1) -
; not committed, regenerated fresh from po/*.po on every build. See the
; [Registry] section below for what it's used for.
...
Root: HKCU; Subkey: "Software\Classes\Virtaal.XliffFile"; ValueType: string; ValueName: ""; ValueData: "{cm:XliffFileType}"; Tasks: fileassoc; Flags: uninsdeletekey
...
Root: HKCU; Subkey: "Software\Classes\Virtaal.QmFile"; ValueType: string; ValueName: ""; ValueData: "{cm:QmFileType}"; Tasks: fileassoc; Flags: uninsdeletekey
...
Root: HKCU; Subkey: "Software\Classes\Virtaal.TbxFile"; ValueType: string; ValueName: ""; ValueData: "{cm:TbxFileType}"; Tasks: fileassoc; Flags: uninsdeletekey
...
Root: HKCU; Subkey: "Software\Classes\Virtaal.TmxFile"; ValueType: string; ValueName: ""; ValueData: "{cm:TmxFileType}"; Tasks: fileassoc; Flags: uninsdeletekey
...
Root: HKCU; Subkey: "Software\Classes\Virtaal.QphFile"; ValueType: string; ValueName: ""; ValueData: "{cm:QphFileType}"; Tasks: fileassoc; Flags: uninsdeletekey
