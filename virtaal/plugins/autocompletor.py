        ...
        def add_widgets():
            if hasattr(self, 'lastunit'):
                if self.lastunit.hasplural():
                    # self.lastunit.target for a plural unit is a
                    # multistring, which subclasses str - iterating it
                    # directly yields individual characters of just the
                    # first plural form, not each form's own text.
                    # target.strings is each form. unitview.py's own
                    # plural handling already uses target.strings for the
                    # same reason.
