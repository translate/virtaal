...
class LanguageSelectDialog(object):
    ...
    def _init_treeviews(self):
        ...
        def searchfunc(model, column, key, iter, search_data):
            if  model.get_value(iter, 0).lower().startswith(key.lower()) or \
                model.get_value(iter, 1).lower().startswith(key.lower()):
                return False
            return True
        self.tvw_sourcelang.set_search_equal_func(searchfunc, None)
        self.tvw_targetlang.set_search_equal_func(searchfunc, None)
